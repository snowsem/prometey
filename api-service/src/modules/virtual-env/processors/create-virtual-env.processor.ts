...
import {InjectQueue, Processor, Process, OnQueueCompleted, OnQueueFailed} from '@nestjs/bull';
...
export class CreateVirtualEnvProcessor {
  ...
  public success(job: Job, result: any) {
    this.logger.debug(`Completed: ${JSON.stringify(job)} ${JSON.stringify(result)}`);
  }

  @OnQueueFailed()
  public failed(job: Job, err: Error) {
    this.logger.error(`Error Job: ${JSON.stringify(job)} ${JSON.stringify(err)}`);
  }
}
